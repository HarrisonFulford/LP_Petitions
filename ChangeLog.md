...
## 10 — S2 live deploy to Base Sepolia

- **S2 executed (real broadcast).** Ran `script/Deploy.s.sol:Deploy` with `--broadcast` from the funded deployer `0xC29E461Ff6a0d9f6516bEfcCA07e13e5b2f38eB4`; on-chain execution succeeded on Base Sepolia (chainId 84532). Cost ~0.0000258 test ETH at ~0.011 gwei.
- **Deployed addresses (also in gitignored `deployments/base-sepolia.json`):** `LPPetition` `0x0eD33705a3c847791325876d6F2815B550eeBE1C`, `MockSPCX` `0x1934BEa899639Fdb0665f93385D658B2B1c5E045`, mock SPCX/USD feed `0x2d6D7651319D018F655a6E3eB7fE962D05702957`. Real WETH9 `0x4200…0006` and real ETH/USD feed `0x4aDC6769…7cb1` referenced (not deployed).
- **On-chain state verified:** both feeds registered; demo petition id `1` open (SPCX/WETH, fee 3000, $5k threshold, `petitionCount == 1`, hypothetical TVL 0 pre-signing); 1,000 SPCX minted to the deployer; all three contracts return non-zero codesize.
...
- **Remaining S2 follow-ups (not blocking):** hand the address book to Ian for `web/` config (the deployments file is gitignored, so it does not sync via git); optionally seed additional demo/signer wallets with SPCX (`DEMO_WALLETS=` re-run or direct `mint`) — WETH comes from wrapping faucet ETH in the app.
