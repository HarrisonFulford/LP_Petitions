...
## 10 — S2 live deploy to Base Sepolia (NVDA)

- **S2 executed (real broadcast).** Ran `script/Deploy.s.sol:Deploy` with `--broadcast` from the funded deployer `0xC29E461Ff6a0d9f6516bEfcCA07e13e5b2f38eB4`; on-chain execution succeeded on Base Sepolia (chainId 84532). Cost ~0.0000258 test ETH at ~0.011 gwei per deploy.
- **Demo asset is NVDA.** An initial deploy used the pre-migration SPCX build (now orphaned on-chain, harmless); after the repo's SPCX→NVDA migration landed, re-deployed with the current `MockNVDA` code so the live contracts match the codebase + `web/` app. The addresses below are the **current/canonical** NVDA set.
- **Deployed addresses (also in gitignored `deployments/base-sepolia.json`):** `LPPetition` `0x2E52Db1e00BA29Dc19373A3B3D4879F3fe153C31`, `MockNVDA` `0xfD51A2794b38BAa8C19412429ACf668843E4c04C` (symbol `NVDA`), mock NVDA/USD feed `0x88d1D29733312FCc79440F37FEa3F7F70b70530a`. Real WETH9 `0x4200…0006` and real ETH/USD feed `0x4aDC6769…7cb1` referenced (not deployed).
- **On-chain state verified:** both feeds registered; demo petition id `1` open (NVDA/WETH, fee 3000, $5k threshold, `petitionCount == 1`, hypothetical TVL 0 pre-signing); 1,000 NVDA minted to the deployer; all three contracts return non-zero codesize.
- **Operator notes (env, not code):** deploy initially reverted on `vm.envUint("PRIVATE_KEY")` — the `.env` key was missing the `0x` prefix (added); the RPC URL initially pointed at `eth-mainnet` (chainId 1) and was corrected to `base-sepolia` (chainId 84532). `contracts/.env` stays gitignored.
- **Remaining S2 follow-ups (not blocking):** wire the canonical NVDA address book into `web/` config (the deployments file is gitignored, so it does not sync via git); optionally seed additional demo/signer wallets with NVDA (`DEMO_WALLETS=` re-run or direct `mint`) — WETH comes from wrapping faucet ETH in the app.

## 11 — F3 commitment signing (functional Permit2 flow)

- **Wired `web/` to the canonical NVDA deployment** via `web/.env.local` (gitignored): `NEXT_PUBLIC_LP_PETITION_ADDRESS` / `NEXT_PUBLIC_MOCK_NVDA_ADDRESS` / `NEXT_PUBLIC_MOCK_NVDA_USD_AGGREGATOR_ADDRESS`.
- **Implemented F3 end-to-end.** New `web/src/lib/contracts/permit2.ts` (AllowanceTransfer ABI subset + EIP-712 domain/`PermitBatch` types + `buildPermitBatch`) and `erc20.ts` (approve/allowance/balanceOf). Rebuilt `commitment-sign-panel.tsx` to run the real flow: read sorted `token0/token1` via `getPetition` → ERC20 `approve(PERMIT2)` where missing → read Permit2 nonces → sign `PermitBatch` (spender = `LPPetition`) → submit `PERMIT2.permit()` → `sign(id, amount0, amount1)` → mirror to F2. Added `getPetition` to the LPPetition ABI.
- **Key correctness facts (now reflected in docs):** the contract reads the *stored* `PERMIT2.allowance(signer, token, address(this))`, so the permit must be **registered on-chain** (a bare signature is not read); there is **no custom commitment EIP-712** (`sign()` verifies no signature); `amount0/amount1` map to the petition's **sorted** tokens, not the display pair order.
- **Docs reconciled:** rewrote F3 + the S0 note + the shared S0 bullet in `fullstack.md` and `contract.md` to drop the nonexistent "commitment typed-data" / mandatory server-side signature verification framing.
- **Status:** `tsc --noEmit` + `eslint` green for the changed files (`next typegen` needs Node ≥20; the local shell has 18, so full `npm run typecheck` is environment-blocked, not code-blocked).

## 12 — F6 auto-executor (reactive + cron)

- **Shared executor module** (`web/src/lib/executor/execute.ts`): `maybeExecutePetition` (backend-TVL gate → `simulateContract` → `writeContract` `execute(id, calls)` → wait receipt → `markExecuted` + parse `Executed`) and `runSweep` (re-checks all open petitions). Idempotent via an in-process lock **plus** the contract's single-exec guard (authoritative); on-chain reverts (`BelowThreshold`/`PetitionNotOpen`) are mapped to clean skips.
- **Triggers:** reactive path wired into `POST /api/petitions/[id]/commitments/confirm` via `after()` (fires post-response, non-blocking); **Vercel Cron** safety net at `GET /api/cron/sweep` (every minute, `web/vercel.json`, `CRON_SECRET`-gated). Manual fallback at `POST /api/petitions/[id]/execute`.
- **Server-only signer:** `web/src/lib/web3/executor-client.ts` builds a viem wallet client from `EXECUTOR_PRIVATE_KEY` (never `NEXT_PUBLIC`); returns null (clean skip) when unset. New env: `EXECUTOR_PRIVATE_KEY`, `CRON_SECRET`, `UNISWAP_API_KEY`, `UNISWAP_API_BASE_URL`.
- **Store:** added `markExecuted(id)` (memory + Postgres) to flip status to `executed` after a successful execute. ABI: added `execute`, `isThresholdMet`, `hypotheticalTvlUsdE18`, and `Executed`/`PositionMinted` events.
- **Uniswap Swap-API integration point** (`web/src/lib/uniswap/swap-api.ts`): `buildBalancingSwapCalls` returns `[]` for the MVP (the contract mints at the Chainlink ratio, so proportional commits need no swap). **Known gap:** the live Swap-API calldata path (fetch `/quote`+`/swap`, encode UniversalRouter calldata) is scaffolded but disabled until validated against the deployed router — so the Uniswap **API key** is not yet exercised in the execute path. Flag for the Uniswap track requirement.
- **Verified:** `tsc --noEmit` + `eslint` clean; runtime smoke (`GET /api/cron/sweep` → `{checked:0,results:[]}`, manual execute on a missing id → 404). No real on-chain `execute()` fired yet (needs a signer with NVDA+WETH + Permit2 allowance clearing the $5k threshold — that's S3).
